...
def run_pipeline(data_path):
    ...
    return final_df, ml_pipeline

def create_preprocessing_pipeline():
    """
    Creates the full preprocessing pipeline for converting raw customer data.
    This pipeline is used during inference to transform raw inputs before model prediction.
    
    Returns:
        Pipeline: A sklearn Pipeline that handles:
...
def create_ml_pipeline():
    """
    Creates the ML pipeline for feature transformation (WoE + Scaling).
    This pipeline is used during inference to transform preprocessed data before model prediction.
